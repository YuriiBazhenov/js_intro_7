...
const toInitials = string => string.split(' ').map(value => value[0].toUpperCase() + '.').join('')
...
const hasNumbers = string => string.split('').some(value => Number(value) === parseInt(value))
...
const isArraySumEvenOrOdd = array => array.reduce((a, b) => (a + b), 0) % 2 === 0 ? 'even' : 'odd'
...
const reverseWords = string => {
    ...
    return reverse.split(' ').map(value => value).reverse().join(' ')
