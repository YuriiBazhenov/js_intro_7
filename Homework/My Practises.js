...
function lastNeg(array) {
    for (const neg of array.reverse()) {
        if (neg < 0) return neg;
    }
}
...
longest count of characters, always return the first 
occurrence.
...
function firstLongest(array) {
    ...
    for (const word of array) {
        if (word.length > firstL.length) firstL = word;
    }
    return firstL;
}
...
function commonElements(arr1, arr2) {
    ...
    for (const arr of arr1) {
        if (arr2.includes(arr)) common.push(arr);
    }
return common;
}
